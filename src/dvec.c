...
/*
    This function figures out where to crop i guess 
                                ...
                                const int omaptrinc, const int nmaptrinc)
{
    ...
    *crop0 = 0U;
    *crop1 = chunk_n1;
    //We dont crop if the cropping are is too small, so cropping isnt necessary
...
/*
    Removes the unessesary pixels at the edges 
    Compare to e.g. photoshops crop function

    Note it can be disabled in puzzledigg with the -c command
                                ...
                                PuzzleView * const view)
{
    ...
    printf("\nAutocropping cropx0: %d cropy0: %d", cropx0,cropy0);
    printf("\nAutocropping cropx1: %d cropy1: %d", cropx1,cropy1);
    maptr = view->map;
    y = cropy0;
    do {
        x = cropx0;
        do {
            ...
            *maptr++ = PUZZLE_VIEW_PIXEL(view, x, y);
            //printf("test %d ",PUZZLE_VIEW_PIXEL(view,x,y));
        } while (x++ != cropx1);
    } while (y++ != cropy1);

    //Updateing width and height to exclude the unesessary pixels
    printf("\nAutocropping before update view.width %d view.height %d", view->width, view->height);
    view->width = cropx1 - cropx0 + 1U;
    view->height = cropy1 - cropy0 + 1U;
    view->sizeof_map = (size_t) view->width * (size_t) view->height;
    printf("\nAutocropping after update view.width %d view.height %d", view->width, view->height);
...
/*
Calculates a value for all pixels based on their rgbvalues and stores it in an array
*/
                                       ...
                                       gdImagePtr gdimage)
{
    ...
    printf("getview from gdimage height: %d width %d \n", x1, y1);
    ...
    maptr = view->map;
    x = x1;
    printf("Calculating pixelvalues: \n");
    int loopcounter;
    loopcounter = 0;
...
/*
This function calculates the average graylevel of one of the boxes
So it's run 9*9 times, once for each square
                                ...
                                const unsigned int height)
{
    ...
    //printf("puzzle_get_avglvl x:%d y: %d \n", x, y);
    double lvl = 0.0;
    ...
    //printf("\ngetaveragelevel %f ",lvl / (double) (width * height) );
                               ...
                               const unsigned int lambdas)
{
    ...
    /*
        SUCC = n+1
        PRED = n-1
        MIN is probablt selfexplanatory
    */
    xshift = (width -
    ...
    printf("\npuzzle_fill_avglgls xshift: %f yshift %f", xshift, yshift);
    printf("width:%f height:%f",width,height);
    printf("\npuzzle_fill_avglgls lamdas: %d", lambdas);
    ...
    do {
        ...
        do {
            ...
            if (px > 0U && py > 0U) {
                //p seems to be some kind of control value
                avglvl = puzzle_get_avglvl(view, xd, yd, px, py);
              //  printf(" returnvalue from puzzle_get_avglvl: %f ", avglvl);
            } else {
                avglvl = 0.0;
            }
            printf("\nlwidth: %u  lheight %u  x:%f  y:%f  lx:%u  ly:%u  xd:%u yd %u px %u py %u avglvl %f", lwidth,lheight,x,y,lx,ly,xd,yd,px,py, avglvl);
            //printf(" storing avglvl: %f\n", avglvl);
